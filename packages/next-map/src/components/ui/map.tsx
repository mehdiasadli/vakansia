/** biome-ignore-all lint/suspicious/noShadowRestrictedNames: It is automatically generated */
/** biome-ignore-all lint/correctness/useExhaustiveDependencies: need to use */
/** biome-ignore-all lint/style/noNestedTernary: need to use */
import "leaflet-draw/dist/leaflet.draw.css";
import "leaflet/dist/leaflet.css";

import { Button } from "@vakansia/next-ui/ui/button";
import { ButtonGroup } from "@vakansia/next-ui/ui/group";
import {
	DropdownMenu,
...
import { cn } from "@vakansia/next-ui/utils";
import type {
	Circle,
	CircleMarker,
	DivIconOptions,
...
function MapCircle({
	...
	return (
		<LeafletCircle
			className={cn("fill-foreground stroke-2 stroke-foreground", className)}
			{...props}
		/>
	);
}
...
function MapLocateControl({
	...
	const stopLocationEffect = useEffectEvent(() => stopLocating());

	useEffect(() => stopLocationEffect, []);
	...
	return (
		<MapControlContainer className={cn("right-1 bottom-1", className)}>
			...
			{position && (
				<MapMarker icon={<MapLocatePulseIcon />} position={position} />
			)}
		</MapControlContainer>
	);
}
